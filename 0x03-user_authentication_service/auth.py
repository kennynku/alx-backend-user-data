...
def _hash_password(password: str) -> str:
    """ Taking in string arg, converting to unicode Returns
...
class Auth:
    ...
    def valid_login(self, email: str, password: str) -> bool:
        """ Checking if user pswd is valid, locating by email """
        try:
    ...
    def create_session(self, email: str) -> str:
        """ Creating session ID using UUID, finds user by email """
        try:
            found_user = self._db.find_user_by(email=email)
        except NoResultFound:
            return None
    ...
    def destroy_session(self, user_id: str) -> None:
            """ Updating user's session_id to None"""
            if user_id is None:
                return None
            try:
    ...
    def get_reset_password_token(self, email: str) -> str:
        """ Finding user by email, updates user's reset_toke with UUID """
        try:
            found_user = self._db.find_user_by(email=email)
        except NoResultFound:
            raise ValueError
    ...
    def update_password(self, reset_token: str, password: str) -> None:
        """ Using the reset_token to find the corresponding user.
